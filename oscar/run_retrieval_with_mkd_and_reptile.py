...
class RetrievalDataset(Dataset):
    ...
    def tensorize_example(self,
        ...
        input_ids = self.tokenizer.convert_tokens_to_ids(tokens)  # [101, 10962, 1998, 11546, ..., 1012, 102, 2813, 2111, 2158, ..., 7223, 102, 0, 0, 0, ..., 0]
        ...
        if att_mask_type == "CLR":
            ...
            attention_mask_txt = [1] * seq_len + [0] * seq_padding_len + [0] * img_len + [0] * img_padding_len  # [1]*44 + [0]*26 + [0]*39 + [0]*11
            attention_mask_img = [0] * seq_len + [0] * seq_padding_len + [1] * img_len + [0] * img_padding_len  # [0]*44 + [0]*26 + [1]*39 + [0]*11
        else:
    ...
    def __getitem__(self, index):
        if self.is_train:  # is_train=True
            ...
            example = self.tensorize_example(text_a=caption, img_feat=feature, text_b=od_labels)  # input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img
            ...
            if random.random() <= 0.5:
                ...
                example_neg = self.tensorize_example(text_a=caption_neg, img_feat=feature, text_b=od_labels)  # input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img
            else:
                ...
                example_neg = self.tensorize_example(text_a=caption, img_feat=feature_neg, text_b=od_labels_neg)  # input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img

            example_pair = tuple(list(example) + [1] + list(example_neg) + [0])
            return index, example_pair  # index, (input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img, [label], input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img, [label])
        else:  # is_train=False
            ...
            example = self.tensorize_example(caption, feature, text_b=od_labels)  # input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img

            label = 1 if img_key == cap_idxs[0] else 0
            return index, tuple(list(example) + [label])  # index, (input_ids, attention_mask, segment_ids, img_feat, attention_mask_txt, attention_mask_img, [label])
...
def train(args, train_dataset, val_dataset, student_model, teacher_model, tokenizer):
    ...
    [
        [
            ...
            (
                ...
                torch.Size([16, 178]),  # attention_mask_txt
                torch.Size([16, 178]),  # attention_mask_img
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            inputs = {
                'input_ids': torch.cat((batch[0], batch[7]), dim=0),  # input_ids: (32, 70)
                'attention_mask': torch.cat((batch[1], batch[8]), dim=0),  # input_mask: (32, 120)
                'token_type_ids': torch.cat((batch[2], batch[9]), dim=0),  # segment_ids: (32, 70)
                'img_feats': torch.cat((batch[3], batch[10]), dim=0),  # img_feats: (32, 50, 2054)
                #
                'attention_mask_txt': torch.cat((batch[4], batch[11]), dim=0),  # input_mask: (32, 120)
                'attention_mask_img': torch.cat((batch[5], batch[12]), dim=0),  # input_mask: (32, 120)
                #
                'labels': torch.cat((batch[6], batch[13]), dim=0)  # label_id: (32, 1)
            }
            ...
            # text only
            fast_outputs_txt = fast_model(input_ids=inputs['input_ids'],  # (32, 70)
                                          token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                          attention_mask=inputs['attention_mask_txt'],  # (32, 120)
                                          # labels=inputs['labels'],  # (32, 1)
                                          img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
            # image only
            fast_outputs_img = fast_model(input_ids=inputs['input_ids'],  # (32, 70)
                                          token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                          attention_mask=inputs['attention_mask_img'],  # (32, 120)
                                          # labels=inputs['labels'],  # (32, 1)
                                          img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
            # text + image
            fast_outputs = fast_model(input_ids=inputs['input_ids'],  # (32, 70)
                                      token_type_ids=inputs['token_type_ids'],  # (32, 70)
            ...
            with torch.no_grad():
                # text only
                teacher_outputs_txt = teacher_model(input_ids=inputs['input_ids'],  # (32, 70)
                                                    token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                                    attention_mask=inputs['attention_mask_txt'],  # (32, 120))
                                                    img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
                # image only
                teacher_outputs_img = teacher_model(input_ids=inputs['input_ids'],  # (32, 70)
                                                    token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                                    attention_mask=inputs['attention_mask_img'],  # (32, 120))
                                                    img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
                # text + image
                teacher_outputs = teacher_model(input_ids=inputs['input_ids'],  # (32, 70)
                                                token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                                attention_mask=inputs['attention_mask'],  # (32, 120))
                                                img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
                '''
                ...
                '''
            # L_task_specific
            task_specific_loss, fast_logits = fast_outputs[0:2]  # torch(数) (32, 2)
            fast_logits_txt = fast_outputs_txt[0]
            fast_logits_img = fast_outputs_img[0]
            teacher_logits = teacher_outputs[0]  # (32, 2)
            ...
            # text only
            student_outputs_txt = student_model(input_ids=inputs['input_ids'],  # (32, 70)
                                                token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                                attention_mask=inputs['attention_mask_txt'],  # (32, 120)
                                                # labels=inputs['labels'],  # (32, 2)
                                                img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
            # image only
            student_outputs_img = student_model(input_ids=inputs['input_ids'],  # (32, 70)
                                                token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                                attention_mask=inputs['attention_mask_img'],  # (32, 120)
                                                # labels=inputs['labels'],  # (32, 2)
                                                img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
            # text + image
            student_outputs = student_model(input_ids=inputs['input_ids'],  # (32, 70)
                                            token_type_ids=inputs['token_type_ids'],  # (32, 70)
                                            attention_mask=inputs['attention_mask'],  # (32, 120)
                                            labels=inputs['labels'],  # (32, 2)
                                            img_feats=inputs['img_feats'])  # 新增: (32, 50, 2054)
            ...
            # L_task_specific
            task_specific_loss, student_logits = student_outputs[0:2]  # torch(数) (32, 2)
            ...
            if global_step % args.logging_steps == 0:
                logger.info("Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), score: {:.4f} ({:.4f})".format(
                    epoch + 1, global_step, student_optimizer.param_groups[0]["lr"], loss, tr_loss / global_step, batch_acc, train_acc / global_step)
                )
